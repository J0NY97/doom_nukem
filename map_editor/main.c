...
int	main(int argc, char **argv)
{
	...
	while (!libui->quit)
	{
		...
		if (!vector_is_empty(doom->spawn.pos))
			gfx_draw_vector(doom->grid.elem->surface, 0xff00ff00, 6, gfx_vector_multiply(doom->spawn.pos, doom->grid.gap));
	// @Note: this is here so that it will be drawn on top of the walls
		if (!vector_is_empty(doom->grid.selected1) && doom->button_draw->state == 1)
		{
			gfx_draw_line(doom->grid.elem->surface, 0xffffff00,
